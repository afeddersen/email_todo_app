...
class Integer(base.AbstractSimpleAsn1Item):
    ...
    def __pow__(self, value, modulo=None): return self.clone(
        pow(self._value, value, modulo))
...
class OctetString(base.AbstractSimpleAsn1Item):
    ...
    def __add__(self, value): return self.clone(
        self._value + self.prettyIn(value))

    def __radd__(self, value): return self.clone(
        self.prettyIn(value) + self._value)
...
class Null(OctetString):
    ...
    subtypeSpec = OctetString.subtypeSpec + \
        constraint.SingleValueConstraint(''.encode())
    ...
    def prettyIn(self, value):
        ...
        for x in value:
            if not isinstance(x, intTypes) or x < 0:
                raise error.PyAsn1Error(
                    'Invalid sub-ID in %s at %s' % (value,
...
class SetOf(base.AbstractConstructedAsn1Item):
    ...
    def _verifyComponent(self, idx, value):
        ...
        if not t.isSameTypeWith(value, matchConstraints=self.strictConstraints):
            raise error.PyAsn1Error(
                'Component value is tag-incompatible: %r vs %r' % (value, t))
        if self.strictConstraints and \
                not t.isSuperTypeOf(value, matchTags=False):
            raise error.PyAsn1Error(
                'Component value is constraints-incompatible: %r vs %r' % (value, t))
...
class SequenceAndSetBase(base.AbstractConstructedAsn1Item):
    ...
    def setComponentByName(self, name, value=None, verifyConstraints=True):
        return self.setComponentByPosition(
            self._componentType.getPositionByName(
                name), value, verifyConstraints
        )
    ...
    def prettyPrintType(self, scope=0):
        ...
        for idx in range(len(self.componentType)):
            ...
            r = '%s = %s\n' % (
                r, self._componentType.getTypeByPosition(
                    idx).prettyPrintType(scope)
            )
        return r + '\n' + ' ' * (scope - 1) + '}'
    ...
    def setComponentByType(self, tagSet, value=None, innerFlag=0,
        ...
        if innerFlag:  # set inner component by inner tagSet
            if t.getTagSet():
                ...
                t = self.setComponentByPosition(
                    idx).getComponentByPosition(idx)
