...
class AbstractSimpleDecoder(AbstractDecoder):
    ...
    def _createComponent(self, asn1Spec, tagSet, value=None):
        if tagSet[0][1] not in self.tagFormats:
            raise error.PyAsn1Error('Invalid tag format %s for %s' % (
                tagSet[0], self.protoComponent.prettyPrintType()))
        if asn1Spec is None:
    ...
    def indefLenValueDecoder(self, fullSubstrate, substrate, asn1Spec, tagSet,
        ...
        while substrate:
            ...
            component, substrate = decodeFun(
                substrate, asn1Spec, allowEoo=True)
            if eoo.endOfOctets.isSameTypeWith(component) and \
                    component == eoo.endOfOctets:
                break
            idx = self._getComponentPositionByType(
...
#    defaultErrorState = stDumpRawValue
    ...
    def __call__(self, substrate, asn1Spec=None, tagSet=None,
        ...
        if debug.logger & debug.flagDecoder:
            debug.logger('decoder called at scope %s with state %d, working with up to %d octets of substrate: %s' % (
        ...
        while state != stStop:
            ...
            if state == stDecodeLength:
                ...
                debug.logger and debug.logger & debug.flagDecoder and debug.logger('value length decoded into %d, payload substrate is: %s' % (
                    length, debug.hexdump(length == -1 and substrate or substrate[:length])))
            ...
            if state == stGetValueDecoderByTag:
                ...
                if debug.logger and debug.logger & debug.flagDecoder:
                    debug.logger('codec %s chosen by a built-in type, decoding %s' % (
                        concreteDecoder and concreteDecoder.__class__.__name__ or "<none>", state == stDecodeValue and 'value' or 'as explicit tag'))
                    debug.scope.push(
            ...
            if state == stGetValueDecoderByAsn1Spec:
                    ...
                    if debug.logger and debug.logger & debug.flagDecoder:
                        ...
                        if asn1Spec.getNegMap():
                            ...
                            for t, v in asn1Spec.getNegMap().items():
                                debug.logger('  %s -> %s' %
                                             (t, v.__class__.__name__))
                        debug.logger('new candidate ASN.1 spec is %s, chosen by %s' % (
                            __chosenSpec is None and '<none>' or __chosenSpec.prettyPrintType(), tagSet))
                else:
                ...
                if __chosenSpec is not None and (
                    ...
                    state = stTryAsExplicitTag
                if debug.logger and debug.logger & debug.flagDecoder:
                    debug.logger('codec %s chosen by ASN.1 spec, decoding %s' % (
                        state == stDecodeValue and concreteDecoder.__class__.__name__ or "<none>", state == stDecodeValue and 'value' or 'as explicit tag'))
                    debug.scope.push(
                        __chosenSpec is None and '?' or __chosenSpec.__class__.__name__)
            if state == stTryAsExplicitTag:
                ...
                debug.logger and debug.logger & debug.flagDecoder and debug.logger('codec %s chosen, decoding %s' % (
                    concreteDecoder and concreteDecoder.__class__.__name__ or "<none>", state == stDecodeValue and 'value' or 'as failure'))
            ...
            if state == stDecodeValue:
                ...
                debug.logger and debug.logger & debug.flagDecoder and debug.logger('codec %s yields type %s, value:\n%s\n...remaining substrate is: %s' % (
                    concreteDecoder.__class__.__name__, value.__class__.__name__, value.prettyPrint(), substrate and debug.hexdump(substrate) or '<none>'))
