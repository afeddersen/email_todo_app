...
class MediaUpload(object):
    ...
    def new_from_json(cls, s):
        ...
        """
        data = json.loads(s)
        # Find and call the right classmethod from_json() to restore the
        # object.
        module = data['_module']
...
class HttpRequest(object):
    ...
    def execute(self, http=None, num_retries=0):
        ...
        resp, content = _retry_request(
            http, num_retries, 'request', self._sleep, self._rand, str(
                self.uri),
...
class BatchHttpRequest(object):
    ...
    def add(self, request, callback=None, request_id=None):
        ...
        if request_id in self._requests:
            raise KeyError(
                "A request with this ID already exists: %s" % request_id)
    ...
    def execute(self, http=None):
        ...
        # If http is not supplied use the first valid one given in the
        # requests.
        if http is None:
        ...
        # credentials should be refreshed and then sent again in a separate
        # batch.
        redo_requests = {}
        redo_order = []
