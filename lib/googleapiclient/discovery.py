...
def _retrieve_discovery_doc(url, http, cache_discovery, cache=None):
    ...
    if 'REMOTE_ADDR' in os.environ:
        actual_url = _add_query_parameter(
            url, 'userIp', os.environ['REMOTE_ADDR'])
...
def createMethod(methodName, methodDesc, rootDesc, schema):
    ...
    def method(self, **kwargs):
        # Don't bother with doc string, it will be over-written by
        # createMethod.
...
class Resource(object):
    ...
    def _set_service_methods(self):
        self._add_basic_methods(
            self._resourceDesc, self._rootDesc, self._schema)
        self._add_nested_resources(
            self._resourceDesc, self._rootDesc, self._schema)
    ...
    def _add_basic_methods(self, resourceDesc, rootDesc, schema):
        ...
        if resourceDesc == rootDesc:
            ...
            self._set_dynamic_attr(
                'new_batch_http_request', new_batch_http_request)
