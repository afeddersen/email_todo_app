    ...
    * ``default_body_encoding`` is set to 'UTF-8' by default, it exists to
    ...
    def _status_code__set(self, code):
        try:
            ...
            self._status = '%d %s' % (
                code, status_generic_reasons[code // 100])
...
#         #self.content_length = None
    ...
    json = json_body = property(
        _json_body__get, _json_body__set, _json_body__del)
    ...
    def decode_content(self):
        ...
        if content_encoding == 'gzip':
            ...
            gzip_f = GzipFile(filename='', mode='r',
                              fileobj=BytesIO(self.body))
        ...
        * Range               (406 Partial Content; only on GET, HEAD)
        ...
        if (
            ...
            if content_range is None:
                iter_close(self._app_iter)
                body = bytes_(
                    "Requested range not satisfiable: %s" % req.range)
                headerlist = [
