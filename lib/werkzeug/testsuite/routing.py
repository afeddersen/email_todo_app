...
class RoutingTestCase(WerkzeugTestCase):
    ...
    def test_alias_redirects(self):
        ...
        ensure_redirect('/users/page-1.html',
                        '/users/?foo=bar', {'foo': 'bar'})
    ...
    def test_double_defaults(self):
        for prefix in '', '/aaa':
            m = r.Map([
                r.Rule(prefix + '/',
                       defaults={'foo': 1, 'bar': False}, endpoint='x'),
                r.Rule(prefix + '/<int:foo>',
                       defaults={'bar': False}, endpoint='x'),
                r.Rule(prefix + '/bar/',
                       defaults={'foo': 1, 'bar': True}, endpoint='x'),
